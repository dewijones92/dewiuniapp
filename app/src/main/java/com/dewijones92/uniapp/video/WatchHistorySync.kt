 ...
 * a YouTube video here (podcasts are audio-only, so they're skipped), and when
 * signed out the reporter no-ops cheaply. Reports on a new video, on finishing,
 * and roughly every [REPORT_INTERVAL_MS]; a finished video is reported once.
 */
    ...
    fun start() {
        scope.launch {
            ...
            playback.state.collect { state ->
                ...
                scope.launch { history.reportProgress(videoId, positionSec, lengthSec, finished) }
            }
        }
    }
